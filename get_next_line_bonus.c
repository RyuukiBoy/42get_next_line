...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/04 20:49:25 by oait-bad          #+#    #+#             */
/*   Updated: 2022/12/04 20:55:50 by oait-bad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE < 0 || fd > OPEN_MAX)
		return (NULL);
	str[fd] = ft_find_nl(fd, str[fd]);
	if (!str[fd])
		return (NULL);
	ret = ft_line_rt(str[fd]);
	str[fd] = ft_after_nl(str[fd]);
	return (ret);
}
